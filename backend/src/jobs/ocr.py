...
Tras el resultado principal, `run_ocr` dispara la comparativa original-vs-realzada (S2.10) en su
PROPIA transacción, separada de la principal (que ya se confirmó al llegar ahí): cualquier fallo de
la comparativa (interruptor, realce, segunda lectura o persistencia) queda contenido y jamás puede
tocar el resultado principal (spec S2.9/S2.10 C5). `run_ocr_comparison` es pública (no `_privada`)
porque también la reutiliza `jobs.ocr_backfill` (backfill retroactivo sobre facturas ya procesadas,
mismo paquete `jobs`, sin duplicar esta lógica ni invertir la dirección de dependencias jobs->ocr).
"""
    ...
    try:
        async with tenant_session(tenant_id, company_id) as session:
            ...
            await comparison_repository.upsert_comparison_run(
                ...
                original_reading=comparison.serialize_reading(
                    resolved_original, verdict.original_analysis
                ),
                enhanced_reading=comparison.serialize_reading(
                    enhanced_reading, verdict.enhanced_analysis
                ),
