...
describe('upload contract (R-011)', () => {
  it('acepta una respuesta 201 con id de fichero', async () => {
    await expect(readUploadResult(new Response(JSON.stringify({ id: 'file-1' }), { status: 201 }))).resolves.toEqual({ id: 'file-1' })
  })
  ...
  it('conserva el camino explícito de duplicado propio en 409', async () => {
    await expect(readUploadResult(new Response(JSON.stringify({ duplicate_of: 'file-original' }), { status: 409 }))).resolves.toEqual({ id: 'file-original' })
  })
})
