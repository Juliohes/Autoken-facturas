...
/** Convierte un valor de tramo (número o string del OCR) al string editable. */
function taxCellToInput(value: number | string | null | undefined): string {
...
export function reviewToFormState(fields: ReviewFields): ConfirmFormState {
  return {
    ...
    net_amount: toInput(fields.net_amount),
    tax_amount: toInput(fields.tax_amount),
    total_amount: toInput(fields.total_amount),
    irpf_amount: '',
...
export function formStateToConfirmBody(
  ...
  return {
    ...
    net_amount: toBody(form.net_amount),
    tax_amount: toBody(form.tax_amount),
    total_amount: toBody(form.total_amount),
    irpf_amount: toBody(form.irpf_amount),
    tax_lines: form.tax_lines.map((line) => ({
      iva_pct: toBody(line.iva_pct),
      base: toBody(line.base),
