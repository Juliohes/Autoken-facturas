...
| R-011 | VERIFYING | R-010 | Frontend acepta solo `201` con `id` en `readUploadResult`, conserva el `409 duplicate_of`; backend declara `201`, deja `pending_ocr` y encola tras commit. 5 tests de contrato frontend y 407 tests frontend pasan; tests backend requieren Redis local/staging. |
